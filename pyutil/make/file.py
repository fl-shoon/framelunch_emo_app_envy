...
class File():
    ...
    def hash(self):
        if self._hash == None:
            self._hash = Md5().hash(self.fname)
        return self._hash
    ...
    def scrape(self):
        ...
        for lno, line in enumerate(self.lines()):
            ls = line.lstrip()
            has_kw = any(kw and kw in line for _,kw,_ in scrapers)
            if not has_kw and ls.startswith(self.cmt):
                doc.append(ls[4:].rstrip())
                continue
            for regex, kw, callback in scrapers:
                if kw and kw not in line: continue
                log.debug(f"{kw} IN {line!r}?")
                if m := regex.match(line):
                    added += 1
                    callback(doc=doc[:], fname=self.fname, lno=lno+1, **m.groupdict())
                elif kw:
                    exit(f"Coudn't parse {kw} definition.\n{line}\n{regex.pattern}\n{self.fname}:{lno+1}")
            doc = []
